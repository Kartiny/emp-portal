...
// Helper: get department_id from supervisor uid
async function getDepartmentIdBySupervisorUid(uid: number): Promise<number | null> {
...
export async function POST(req: Request) {
  try {
    ...
    const departmentId = await getDepartmentIdBySupervisorUid(uid);
    if (!departmentId) {
      return NextResponse.json({ error: 'Supervisor has no department' }, { status: 404 });
    }
    ...
    console.error('Supervisor dashboard API error:', err);
