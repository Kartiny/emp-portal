...
export async function GET(req: Request) {
  try {
    ...
    console.log('🔍 Fetching approved leave requests for approver UID:', uid);
    ...
    // Find leave requests that are approved
    const leaveRequests = await (client as any).execute(
      'hr.leave',
      'search_read',
      [[
        ['state', '=', 'validate'], // Approved requests
        '|',
    ...
    console.log('📋 Found approved leave requests:', leaveRequests.length);
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: error.message || 'Failed to fetch approved leave requests' 
      },
      { status: 500 }
    );
  }
