...
export async function GET(req: Request) {
  try {
    ...
    console.log('🔍 Fetching approved expense requests for approver UID:', uid);
    ...
    // Find expense requests that are approved
    const expenseRequests = await (client as any).execute(
      'hr.expense',
      'search_read',
      [[
        ['state', '=', 'approved'], // Approved requests
        '|',
    ...
    console.log('📋 Found approved expense requests:', expenseRequests.length);
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: error.message || 'Failed to fetch approved expense requests' 
      },
      { status: 500 }
    );
  }
